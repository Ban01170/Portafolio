...
def check (test_str):
       ...
       if re.search(pattern, test_str):
            print ('Invalid char in command %r only chars in brackets are allowed [.acflst*- ]\n' % (test_str, ))
            print ('Char not permitted.')

       else:
              try:
                     output = rum_cmd(cmd, get_output=True, stop_on_error=True)
                     print (output)
              except OSError:
                     print ('Unknown error.')
...
while True:
    ...
    try:
        # cmd = re.split(r'\s+', s)
        cmd = s  # Assuming you meant to use the input directly
        check(cmd)
    except OSError:
        print('Unknown error.')